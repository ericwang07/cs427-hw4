...
void Game::playGame() {
  while (!std::cin.eof()) {
    ...
    if (seedsCaptured > 0) {
      std::cout << "Captured " << seedsCaptured << " seeds"
                << std::endl;
    }
...
void Game::printInfo() {
  ...
  std::cout << "Please enter a pit number for "
            << kalah_.getCurrPlayer() << " (q to quit): ";
}
...
bool Game::askValidMove(int &move) {
  while (true) {
    ...
    if (std::cin.fail()) { // detected non-int
      std::cin.clear();
      string command;
      std::cin >> command;
      if (command == "q") {
...
void Game::executeMove(int move) {
  std::cout << std::endl;
  std::cout << kalah_.getCurrPlayer() << " playing move "
            << move << std::endl;
...
void Game::finishGame() {
  ...
  std::cout << "North Total: " << kalah_.getNorthSeeds()
            << " seeds" << std::endl;
  std::cout << "South Total: " << kalah_.getSouthSeeds()
            << " seeds" << std::endl;
